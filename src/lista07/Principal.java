...
public class Principal {
	...
	static ObjectFile arqListas = new ObjectFile("lista7.dat");
		...
		do {
			...
			opcao = Keyboard
					.menu("Incluir Aluno/Listar Alunos/Excluir Alunos/"
							...
							+ "Incluir Turma/Lista Turmas/Excluir Turma/Matricular Aluno/Listar Alunos Matriculados/Terminar");
			switch (opcao) {
			case 1:
				incluirAluno();
				break;
	...
	static void listarAlunosMatriculados() {
		...
		if (turma == null) {
			...
			System.out.println("CodDisc: " + turma.getCodDisc()	+ " CodTurma: " + turma.getCodTurma() + " NomeDisc: "	+ disciplina.getNomeDisc());
			...
			for (int j = 0; j < alunoVet.length; j++) {
					if (alunoVet[j].getTurmas().contains(turma))
						System.out.printf("%4d  %-30s\n", alunoVet[j].getNumMat(),	alunoVet[j].getNomeAluno());
			}
		}
		Keyboard.waitEnter();
	}
	...
	static void listarTurmas() {
		...
		if (listaTurmas.isEmpty()) {
			...
			while (turma != null) {
				...
				disciplina = listaDisciplinas.retrieve(disciplina);
				turmaAux[i] = new TurmaAux(turma.getCodDisc(), disciplina.getNomeDisc(), turma.getCodTurma());
				turma = it.getNext();
				i++;
			}
			Sort.selecao(turmaAux, new ComparaTurma());
			for (int j = 0; j < turmaAux.length; j++) {
				System.out.printf("%8d  %-22s  %5s\n", turmaAux[j].codDisc,	turmaAux[j].nomeDisc, turmaAux[j].codTurma);
			}
		}
		Keyboard.waitEnter();
	}
	...
	static void excluirDisciplina() {
		Keyboard.clrscr();
			...
			if (disciplina == null) {
				...
				System.out.println("Nome da Disciplina: " + disciplina.getNomeDisc());
				char resp = Keyboard.readChar("Deseja excluir?");
				if (resp == 's') {
	...
	static void incluirDisciplina() {
		...
		do {
			...
			if (listaDisciplinas.contains(disciplina)) {
				...
				String nomeDisc = Keyboard.readString("Entrar com o nome da disciplina: ");
